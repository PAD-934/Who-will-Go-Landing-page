...
function isValidOrderData(data) {
  const required = ['customerName', 'email', 'phone', 'address', 'products', 'totalAmount', 'paymentMethod'];
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 20px; background-color: #f9f9f9;">
          ...
          <table style="width: 100%; margin-top: 20px;">
            <tr style="background-color: #f0f0f0;">
              <td style="padding: 10px; font-weight: bold;">T-Shirt Size:</td>
              <td style="padding: 10px;">${orderData.tshirtSize}</td>
            </tr>
            <tr>
...
function testSetup() {
  ...
  if (CONFIG.SHEET_ID === 'YOUR_GOOGLE_SHEET_ID_HERE') {
    issues.push('⚠️ SHEET_ID not configured. Add your Google Sheet ID to CONFIG.');
  }
  
  // Check Admin Email
  if (CONFIG.ADMIN_EMAIL === 'admin@yourchurch.com') {
    issues.push('⚠️ ADMIN_EMAIL not configured. Update to your actual admin email.');
  }
  ...
  if (issues.length === 0) {
    Logger.log('✅ Setup looks good! Deploy as Web App to get your endpoint URL.');
  } else {
